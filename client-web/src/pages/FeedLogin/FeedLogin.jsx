...
const FeedLogin = (props) => {
                ...
                <div className="bgShadow alignBoxes">
                    <div className="friendInfoBox friendInfo transform">
                        <ul>
                            ...
                            <li className="resolution">“ 하루하루를 성실하게 보내는 개발자입니다. ”</li>
                        </ul>
                    </div>
                </div>
            </Link>
    ...
    return (
        <div>
            <FeedTitle />
            <section>{renderMandalKeyword(keywordArr)}</section>
                    ...
                    {props.userlist != undefined &&
                        props.userlist.map((friendInfo) => {
                            if (friendInfo.id != currentUser && cnt < 4) {
                                cnt++
                                return friendInfoMandals(friendInfo)
                            }
                        })}
                </article>
            </section>
                        ...
                        {popularMandal.map((mandal) => {
                            return (
                                <MiniMandalBox
                                    ...
                                    size="mini"
                                    title={mandal.title}
                                    startDate={mandal.startDate}
                                    endDate={mandal.endDate}
                                    getHeart={true}
                                    userInfo={mandal}
                                    thumbnail={mandal.thumbnailPath}
                                    imagePath={mandal.imagePath}
                                    name={mandal.userName}
                                    heartNum={mandal.heartNum}
